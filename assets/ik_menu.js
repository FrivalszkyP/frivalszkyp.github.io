;(function ( $, window, document, undefined ) {
	...
	Plugin.prototype.onKeyDown = function (event) {
		...
		var $submenu = $current.children('ul');
		var $parentitem = $current.parent('ul').parent('li');
		var $ulParents = $current.parents('ul');
			...
			case ik_utils.keys.right:
				...
				if ($ulParents.length == 1) {
					$current
						...
						.attr('tabindex', '0')
						.focus();
				} else if($ulParents.length > 1 && $submenu.length) {
					plugin.showSubmenu(event); // TODO: code reuse could be better
					$submenu
						.children('li:eq(0)')
						.attr('tabindex', '0')
						.focus();
			...
			case ik_utils.keys.left:
				...
				if ($ulParents.length == 1) {
					$current
						...
						.attr('tabindex', '0')
						.focus();
				} else if($ulParents.length > 2 && $parentitem.hasClass('expandable')) {
					$parentitem
						.removeClass('expanded')
						.attr({
							'tabindex': 0,
