;(function ( $, window, document, undefined ) {
	...
	var pluginName = 'ik_slider',
		defaults = {
			...
			'nowValue': 0,
			'step': 1,
			'instructions': 'Use the right and left or up and down arrow keys to increase or decrease the slider value.'
		};
	...
	Plugin.prototype.onKeyDown = function (event) {
		...
		switch (event.keyCode) {
			...
			case ik_utils.keys.up:
				...
				event.preventDefault();
				value = valueNow + plugin.options.step;
			...
			case ik_utils.keys.end: // currently undefined for me
			case 35:
				event.preventDefault();
				plugin.setValue(plugin.options.maxValue);
				break;

			case ik_utils.keys.home: // currently undefined for me
			case 36:
				event.preventDefault();
				plugin.setValue(plugin.options.minValue);
				break;
		}
		console.log(event.keyCode, ik_utils.keys);
	}
